...
<s:form action="filesAction_showFilesByConditionshare.action" id="condition_form">
			<s:hidden name="#pageResult.url" id="action_url"></s:hidden>
 <div style="overflow:auto; height:120px; display:online;" id="condition_query">
...
<!-- 	                <td> -->
<!-- 						<input type="checkbox" class="ids" name="ids" value="${fid}"/>	 -->
<!-- 	                </td> -->
	                ...
	                <td>
	                    <s:a action="filesAction_updateUI.action?fid=%{fid}">修改</s:a>
	                	<s:a action="filesAction_download.action?fid=%{fid}">下载</s:a>
	                </td>
	                <td>
	                	<c:if test="${user.uid==sessionScope.user.uid}">
	                		<s:a action="filesAction_deleteBySelf.action?fid=%{fid}">删除</s:a>
	                	</c:if>
	                </td>
	            </tr>
            </s:iterator>
